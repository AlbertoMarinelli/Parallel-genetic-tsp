/*
 * This version of fastFlow is under construction.
 * In particular, parallelFor (such as map) are applied instead of Farm.
 */
#ifndef GENTSP_FFTSP_H
#define GENTSP_FFTSP_H
...
template<typename T>
class TSPff : public GeneticAlgorithm {
private:
...
template<typename T>
void TSPff<T>::Run(int chromosomeNumber, int generationNumber, double mutationRate, double crossoverRate,
...
template<typename T>
TSPff<T>::TSPff(Graph<T> &graph): graph(graph) {}

template<typename T>
void TSPff<T>::generatePopulation(int chromosomeNumber, int numVertices, int workers) {
    ...
    double keyValue=0;
    int chunk = chromosomeNumber/workers;

    auto gen_function = [&](const long i) {
        std::mt19937 gen(std::random_device{}());
        std::vector<int> sequence(numVertices);
        std::iota(sequence.begin(), sequence.end(), 0);
        std::shuffle(sequence.begin(), sequence.end(), gen);
        population[i].first = keyValue;
        population[i].second = sequence;
    };

    ff::ParallelFor pf(workers);
    pf.parallel_for(0, chromosomeNumber, 1, chunk, gen_function, workers);
...
//Evaluate population
template<typename T>
void TSPff<T>::evaluate(double &evaluationsAverage, int k) {
    auto start = std::chrono::system_clock::now();
...
//Fitness function
template<typename T>
void TSPff<T>::fitness(double &evaluationsAverage) {
    auto start = std::chrono::system_clock::now();
...
template<typename T>
void TSPff<T>::selection(std::vector<std::pair<double, std::vector<int>>> &intermediatePopulation) {
    auto start = std::chrono::system_clock::now();
    std::uniform_real_distribution<double> distribution(0.0, 1.0);
...
template<typename T>
void TSPff<T>::crossover(std::vector<std::pair<double, std::vector<int>>>& intermediatePopulation, double crossoverRate) {
    auto start = std::chrono::system_clock::now();
...
//Mutation
template<typename T>
void TSPff<T>::mutation(std::vector<std::pair<double, std::vector<int>>> &intermediatePopulation,
                                double mutationRate) {
    auto start = std::chrono::system_clock::now();
...
//Print population
template<typename T>
void TSPff<T>::printIntermediatePopulation(
        std::vector<std::pair<double, std::vector<int>>> &intermediatePopulation) {
...
template<typename T>
void TSPff<T>::printBestSolution() {
    std::cout << "Best solution cost: " << population[0].first << std::endl;
}

template<typename T>
void TSPff<T>::printBestSolution_andPath() {
    int chromosomeSize = graph.getNumVertices();
